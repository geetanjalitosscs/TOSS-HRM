...
@section('body')
    <x-main-layout title="Time">
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        @else
                                            <button 
                                                ...
                                                title="View"
                                                onclick="openViewEntryModal({{ $entry['id'] }}, '{{ $entry['employee_name'] }}', '{{ \Carbon\Carbon::parse($entry['work_date'])->format('Y-m-d') }}', {{ json_encode($entry['notes']) }})"
                                            >
        ...
        @else
        <!-- View Entry Modal (For Non-Main Users) -->
        <x-admin.modal
            id="view-entry-modal"
            title="View Work Entry"
            icon="fas fa-eye"
            maxWidth="md"
            backdropOnClick="closeViewEntryModal()"
        >
            <div class="space-y-4">
                <!-- Date -->
                <div>
            ...
            <!-- Modal Footer -->
            <div class="mt-6 flex justify-end gap-3">
                <button type="button" onclick="closeViewEntryModal()" class="hr-btn-secondary">
                    Close
                </button>
            </div>
        </x-admin.modal>
        ...
        <script>
            ...
            function resetEmployeeWorksheetsFilters() {
                var searchForm = document.getElementById('employee-worksheets-search-form');
                if (searchForm) {
            ...
            @if($isMainUser)
            // Edit Entry Modal Functions (Only for Main User)
            ...
            @else
            // View Entry Modal Functions (For Non-Main Users)
            window.openViewEntryModal = function(entryId, employeeName, workDate, notes) {
                const modal = document.getElementById('view-entry-modal');
                const dateDisplay = document.getElementById('view-date-display');
                const notesInput = document.getElementById('view-notes');
                
                if (!modal || !dateDisplay || !notesInput) {
                    console.error('Modal elements not found');
                    return;
                }
            ...
            window.closeViewEntryModal = function() {
                const modal = document.getElementById('view-entry-modal');
                if (modal) {
                    modal.classList.add('hidden');
                    document.body.style.overflow = '';
