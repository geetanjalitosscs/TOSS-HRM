...
@section('title', 'Claim - Configuration - Add Expense Type')
    ...
    <x-main-layout title="Claim">
        ...
        <div>
            <div class="rounded-b-lg shadow-sm border border-t-0 p-6" style="background-color: var(--bg-card); border-color: var(--border-default);">
                <h2 class="text-sm font-bold mb-5" style="color: var(--text-primary);">Add Expense Type</h2>

                <form id="addExpenseTypeForm" class="space-y-6" novalidate>
                    <!-- Name -->
                    <div>
                        <label for="expenseTypeName" class="block text-xs font-medium mb-1" style="color: var(--text-primary);">
                        ...
                        <input
                            ...
                            placeholder=""
                        >
                        <p id="expenseTypeNameError" class="mt-1 text-xs hidden" style="color: #dc3545;">Required</p>
                    </div>

                    <!-- Description -->
                    <div>
                        <label for="expenseTypeDescription" class="block text-xs font-medium mb-1" style="color: var(--text-primary);">Description</label>
                        <textarea
                    ...
                    <div class="flex items-center gap-3">
                        <label for="expenseTypeActive" class="block text-xs font-medium" style="color: var(--text-primary);">Active</label>
                        <div class="relative cursor-pointer" onclick="document.getElementById('expenseTypeActive').click()">
                            <input type="checkbox" class="sr-only" id="expenseTypeActive" checked onchange="toggleActive(this, 'expenseTypeActiveBg', 'expenseTypeActiveCircle')">
                            <div class="w-11 h-6 rounded-full transition-colors duration-200 flex items-center" id="expenseTypeActiveBg" style="background-color: var(--color-hr-primary);">
                                <div class="w-5 h-5 bg-white rounded-full shadow-md transform transition-transform duration-200" id="expenseTypeActiveCircle" style="margin-left: 24px;"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <a
                            href="{{ route('claim.configuration.expenses-types') }}"
        ...
        <script>
            (function () {
                const nameEl = document.getElementById('expenseTypeName');
                const nameErr = document.getElementById('expenseTypeNameError');
                const saveBtn = document.getElementById('expenseTypeSaveBtn');
                ...
                function validate() {
                    const nameInvalid = !nameEl.value.trim();
                    setInvalid(nameEl, nameErr, nameInvalid);
                    return !nameInvalid;
                }
